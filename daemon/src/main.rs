...
struct SystemState {
    dns_servers: Vec<String>, hosts_hash: String, startup_entries: Vec<String>,
    listening_ports: Vec<String>, firewall_profiles: Vec<String>, windows_update_status: String,
}
...
#[derive(Debug, Clone)]
struct Settings {
    dns: bool, hosts: bool, startup: bool, ports: bool, firewall: bool, creds: bool,
    commands: bool, usb: bool, connections: bool, updates: bool, block_scan: bool, block_brute: bool,
    interval_integrity: u64, interval_intrusion: u64,
}

impl Default for Settings {
    fn default() -> Self {
        Self { dns: true, hosts: true, startup: true, ports: true, firewall: true, creds: true, commands: true, usb: true, connections: true, updates: true, block_scan: true, block_brute: true, interval_integrity: 300, interval_intrusion: 10 }
    }
}

struct AppState {
    status: DaemonStatus, events: Vec<Event>, baseline: Option<Baseline>, seed: Vec<u8>,
    tpm_available: bool, pcr_bound: bool, settings: Settings,
...
fn get_encryption_key(seed: &[u8], tpm_available: bool) -> Vec<u8> {
    ...
    hasher.update(seed); hasher.update(machine_id.as_bytes());
        ...
        if hash_path.exists() {
            if let Ok(expected) = fs::read_to_string(&hash_path) {
                if expected.trim() != hash {
                    log_event_direct("agent_tampered", "Binary hash mismatch - tampering detected!", "critical");
                    std::process::exit(1);
                }
            }
        } else {
...
fn lock_seed_in_memory(seed: &[u8]) {
    let locked = seed.to_vec();
...
fn rotate_event_log(data_dir: &PathBuf, max_entries: usize) {
    let log_path = data_dir.join("events.log");
    if log_path.exists() {
        if let Ok(content) = fs::read_to_string(&log_path) {
            let lines: Vec<&str> = content.lines().collect();
            if lines.len() > max_entries {
                let trimmed: Vec<&str> = lines.iter().skip(lines.len() - max_entries).cloned().collect();
                fs::write(&log_path, trimmed.join("\n")).ok();
            }
        }
    }
}
...
fn main() {
    ...
    let state = Arc::new(Mutex::new(AppState {
        status: DaemonStatus { trust_state: "Initialising".into(), token: String::new(), tpm_sealed: tpm_available, pcr_bound, last_check: Utc::now().to_rfc3339(), latest_events: vec![] },
        events: vec![], baseline: Some(baseline), seed, tpm_available, pcr_bound, settings: Settings::default(),
        connection_history: HashMap::new(), known_usb_devices: HashSet::new(), known_connections: HashSet::new(),
    }));
    let state_http = state.clone(); let http_key = enc_key.clone();
    std::thread::spawn(move || serve_http(state_http, &http_key));
    let state_token = state.clone(); std::thread::spawn(move || loop { generate_token(&state_token); std::thread::sleep(Duration::from_secs(30)); });
    let state_creds = state.clone(); std::thread::spawn(move || loop { check_credential_access(&state_creds); std::thread::sleep(Duration::from_secs(15)); });
...
fn serve_http(state: Arc<Mutex<AppState>>, enc_key: &[u8]) {
    ...
    for stream in listener.incoming() {
        if let Ok(mut stream) = stream {
            let mut buffer = [0u8; 8192]; let n = stream.read(&mut buffer).unwrap_or(0);
            let req = String::from_utf8_lossy(&buffer[..n]); let mut guard = state.lock().unwrap();
            let response = if req.contains("POST /settings") {
                ...
                let current = collect_current_state(); let sig = sign_state(&current, &guard.seed);
                guard.baseline = Some(Baseline { state: current, signature: sig }); guard.events.clear();
                ...
                for line in raw.lines() {
                    if let Ok(enc) = base64::engine::general_purpose::STANDARD.decode(line) {
                        if let Some(dec) = decrypt_data(&enc, &key) { decrypted.push_str(&String::from_utf8_lossy(&dec)); decrypted.push('\n'); }
                    }
                }
                ...
                let status = guard.status.clone(); let json = serde_json::to_string(&status).unwrap();
...
fn get_or_create_seed(data_dir: &PathBuf) -> (Vec<u8>, bool, bool) {
    if let Ok((seed, pcr)) = get_tpm_pcr_seed(data_dir) { lock_data_folder(); return (seed, true, pcr); }
    if let Ok(tpm_seed) = get_tpm_seed(data_dir) { lock_data_folder(); return (tpm_seed, true, false); }
    let seed_path = data_dir.join("seed.bin");
    let seed = if seed_path.exists() { fs::read(&seed_path).unwrap_or_else(|_| { let s = random_seed(); fs::write(&seed_path, &s).ok(); s }) } else { let s = random_seed(); fs::write(&seed_path, &s).expect("Failed"); s };
    lock_data_folder(); (seed, false, false)
}
...
fn get_tpm_pcr_seed(data_dir: &PathBuf) -> Result<(Vec<u8>, bool), Box<dyn std::error::Error>> {
    ...
    if pcr_path.exists() { let enc_key = get_encryption_key(&[0u8; 32], true); let enc_data = fs::read(&pcr_path)?; if let Some(seed) = decrypt_data(&enc_data, &enc_key) { return Ok((seed, true)); } }
    let output = Command::new("powershell").args(["-NoProfile", "-Command", "$tpm = Get-Tpm; if($tpm.TpmReady -and $tpm.TpmEnabled -and $tpm.TpmActivated) { $bytes = New-Object Byte[] 32; (New-Object Security.Cryptography.RNGCryptoServiceProvider).GetBytes($bytes); [Convert]::ToBase64String($bytes) } else { Write-Error 'TPM not ready' }"]).output()?;
    if output.status.success() {
        let b64 = String::from_utf8_lossy(&output.stdout).trim().to_string();
        if !b64.is_empty() { let seed = base64::engine::general_purpose::STANDARD.decode(&b64)?; let enc_key = get_encryption_key(&seed, true); let enc_data = encrypt_data(&seed, &enc_key); fs::write(&pcr_path, &enc_data)?; return Ok((seed, true)); }
    }
...
fn get_tpm_seed(data_dir: &PathBuf) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    if output.status.success() {
        let b64 = String::from_utf8_lossy(&output.stdout).trim().to_string();
        if !b64.is_empty() { let seed = base64::engine::general_purpose::STANDARD.decode(&b64)?; fs::write(&tpm_seed_path, &seed)?; return Ok(seed); }
    }
...
fn generate_token(state: &Arc<Mutex<AppState>>) {
    ...
    let mut mac = HmacSha256::new_from_slice(&guard.seed).unwrap(); mac.update(&counter.to_be_bytes());
    guard.status.token = hex::encode(mac.finalize().into_bytes());
    guard.status.tpm_sealed = guard.tpm_available; guard.status.pcr_bound = guard.pcr_bound;
}

fn check_new_connections(state: &Arc<Mutex<AppState>>) {
    let mut guard = state.lock().unwrap(); if !guard.settings.connections { return; }
    if let Ok(o) = Command::new("netstat").args(["-ano", "-p", "TCP"]).output() {
        for l in String::from_utf8_lossy(&o.stdout).lines().skip(4) {
            ...
            if p.len() >= 5 && p[3] == "ESTABLISHED" && p[4].parse::<u32>().is_ok() {
                ...
                if !guard.known_connections.contains(&conn) { guard.known_connections.insert(conn.clone()); let pid = p[4].parse::<u32>().unwrap_or(0); let proc_name = get_process_name(pid); if !proc_name.is_empty() && !is_system_process(&proc_name) { log_event(&mut guard, "new_connection", &format!("New connection: {} ({})", conn, proc_name), "info"); } }
            }
        }
    }
}
...
fn is_system_process(name: &str) -> bool { ["svchost", "lsass", "csrss", "wininit", "services", "spoolsv", "system", "idle", "registry", "smss", "winlogon"].iter().any(|s| name.to_lowercase().contains(s)) }

fn check_credential_access(state: &Arc<Mutex<AppState>>) {
    let mut guard = state.lock().unwrap(); if !guard.settings.creds { return; }
    let user = std::env::var("USERPROFILE").unwrap_or_default();
    for path in &[format!("{}/.ssh/id_rsa", user), format!("{}/.ssh/id_ed25519", user), format!("{}/.aws/credentials", user), format!("{}/.git-credentials", user)] {
        if let Ok(meta) = fs::metadata(path) { if let Ok(modified) = meta.modified() { if let Ok(dur) = SystemTime::now().duration_since(modified) { if dur.as_secs() < 15 { log_event(&mut guard, "credential_access", &format!("Credential access: {}", path), "critical"); } } } }
    }
}

fn check_suspicious_commands(state: &Arc<Mutex<AppState>>) {
    let mut guard = state.lock().unwrap(); if !guard.settings.commands { return; }
    if let Ok(o) = Command::new("powershell").args(["-NoProfile", "-Command", "Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-PowerShell/Operational'; ID=4104} -MaxEvents 5 -ErrorAction SilentlyContinue | ForEach-Object { $_.Message }"]).output() {
        ...
        for p in &["iex", "invoke-expression", "downloadstring", "frombase64string", "-encodedcommand", "-enc", "reg add", "reg delete", "sc stop", "taskkill /f"] { if text.contains(p) { log_event(&mut guard, "suspicious_command", &format!("Suspicious: {}", p), "warning"); break; } }
    }
}

fn check_usb_devices(state: &Arc<Mutex<AppState>>) {
    let mut guard = state.lock().unwrap(); if !guard.settings.usb { return; }
    if let Ok(o) = Command::new("powershell").args(["-NoProfile", "-Command", "Get-PnpDevice -Class USB -ErrorAction SilentlyContinue | Where-Object { $_.Status -eq 'OK' } | Select-Object -ExpandProperty FriendlyName"]).output() {
        for l in String::from_utf8_lossy(&o.stdout).lines() { let n = l.trim().to_string(); if !n.is_empty() && !guard.known_usb_devices.contains(&n) { guard.known_usb_devices.insert(n.clone()); if n.to_lowercase().contains("storage") || n.to_lowercase().contains("flash") { log_event(&mut guard, "usb_device", &format!("New USB: {}", n), "warning"); } } }
    }
}
...
fn load_or_create_baseline(data_dir: &PathBuf, seed: &[u8], enc_key: &[u8]) -> Baseline {
    ...
    if path.exists() { if let Ok(data) = fs::read(&path) { if let Some(plain) = decrypt_data(&data, enc_key) { if let Ok(b) = serde_json::from_str::<Baseline>(&String::from_utf8_lossy(&plain)) { if sign_state(&b.state, seed) == b.signature { return b; } } } } }
    let s = collect_current_state(); let sig = sign_state(&s, seed); let b = Baseline { state: s, signature: sig };
    let plain = serde_json::to_string(&b).unwrap(); let enc = encrypt_data(plain.as_bytes(), enc_key); fs::write(&path, &enc).ok(); b
}

fn sign_state(state: &SystemState, seed: &[u8]) -> String { let data = serde_json::to_string(state).unwrap(); let mut mac = HmacSha256::new_from_slice(seed).unwrap(); mac.update(data.as_bytes()); hex::encode(mac.finalize().into_bytes()) }

fn diff_states(baseline: &SystemState, current: &SystemState) -> Vec<(String, String)> {
    let mut d = Vec::new();
    let bd: HashSet<&str> = baseline.dns_servers.iter().map(|s| s.as_str()).collect(); let cd: HashSet<&str> = current.dns_servers.iter().map(|s| s.as_str()).collect();
    if bd != cd { d.push(("dns_change".into(), format!("DNS changed"))); }
    if baseline.hosts_hash != current.hosts_hash { d.push(("hosts_change".into(), "Hosts modified".into())); }
    let bs: HashSet<&str> = baseline.startup_entries.iter().map(|s| s.as_str()).collect(); let cs: HashSet<&str> = current.startup_entries.iter().map(|s| s.as_str()).collect();
    let ns: Vec<_> = cs.difference(&bs).collect(); let rs: Vec<_> = bs.difference(&cs).collect();
    if !ns.is_empty() || !rs.is_empty() { d.push(("startup_change".into(), format!("Startup changed"))); }
    for pb in &current.listening_ports { if let Some(ps) = pb.split(':').last() { if let Ok(p) = ps.parse::<u16>() { if RISKY_PORTS.contains(&p) { d.push(("risky_port".into(), format!("Risky port: {}", pb))); } } } }
    let bf: HashSet<&str> = baseline.firewall_profiles.iter().map(|s| s.as_str()).collect(); let cf: HashSet<&str> = current.firewall_profiles.iter().map(|s| s.as_str()).collect();
    if bf != cf { d.push(("firewall_change".into(), "Firewall changed".into())); }
    if baseline.windows_update_status != current.windows_update_status { d.push(("update_change".into(), "Update status changed".into())); }
    d
}
...
fn detect_intrusions(state: &Arc<Mutex<AppState>>) {
    ...
    if let Ok(o) = Command::new("netstat").args(["-ano", "-p", "TCP"]).output() {
        ...
        for l in String::from_utf8_lossy(&o.stdout).lines().skip(4) { let p: Vec<&str> = l.split_whitespace().collect(); if p.len() >= 4 && p[3] == "ESTABLISHED" { if let Some(ip) = p[2].rsplitn(2, ':').nth(1) { if let Some(ps) = p[2].split(':').last() { if let Ok(port) = ps.parse::<u16>() { if !ip.starts_with("127.") && !ip.starts_with("192.168.") && !ip.starts_with("10.") { ip_map.entry(ip.to_string()).or_default().insert(port); } } } } } }
    ...
    if let Ok(o) = Command::new("powershell").args(["-NoProfile", "-Command", "Get-WinEvent -FilterHashtable @{LogName='Security'; ID=4625} -MaxEvents 10 -ErrorAction SilentlyContinue | ForEach-Object { $_.TimeCreated.ToString('o') }"]).output() {
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs(); let recent = String::from_utf8_lossy(&o.stdout).lines().filter_map(|l| chrono::DateTime::parse_from_rfc3339(l.trim()).ok().map(|d| d.timestamp() as u64)).filter(|t| now - t < 60).count();
        if recent >= 5 { log_event(&mut guard, "brute_force", &format!("Brute force: {} logins in 60s", recent), "critical"); if guard.settings.block_brute { let _ = Command::new("powershell").args(["-NoProfile", "-Command", "New-NetFirewallRule -DisplayName 'TS-Block-RDP' -Direction Inbound -LocalPort 3389 -Action Block"]).output(); } }
    }
}

fn log_event(state: &mut AppState, event_type: &str, details: &str, severity: &str) {
    let event = Event { timestamp: Utc::now().to_rfc3339(), event_type: event_type.into(), details: details.into(), severity: severity.into() };
    state.events.push(event.clone()); let lp = PathBuf::from(DATA_DIR).join("events.log");
    let enc_key = get_encryption_key(&state.seed, state.tpm_available); let plain = serde_json::to_string(&event).unwrap();
    let enc = encrypt_data(plain.as_bytes(), &enc_key); let b64 = base64::engine::general_purpose::STANDARD.encode(&enc);
...
fn check_integrity(state: &Arc<Mutex<AppState>>) {
    let mut guard = state.lock().unwrap(); let current = collect_current_state();
    ...
    guard.status.last_check = Utc::now().to_rfc3339(); guard.status.latest_events = guard.events.iter().rev().take(5).cloned().collect();
    if guard.events.iter().filter(|e| e.severity == "warning").count() > 3 { let cur = collect_current_state(); let sig = sign_state(&cur, &guard.seed); guard.baseline = Some(Baseline { state: cur, signature: sig }); guard.events.clear(); guard.status.trust_state = "Trusted".into(); guard.status.latest_events = vec![]; log_event(&mut guard, "auto_heal", "Auto-reset baseline", "info"); }
}